...
mlflow.set_experiment("preproduction_model_training")  # Change if for production
...
# ✅ Updated train-test range
train_df = df[(df["datetime"] >= "2025-04-01") & (df["datetime"] <= "2025-07-10")]
test_df = df[(df["datetime"] >= "2025-07-11") & (df["datetime"] <= "2025-07-25")]
...
# SARIMAX parameters (can tune further if needed)
best_params = {
    'p': 2, 'd': 0, 'q': 0,
    'P': 2, 'D': 0, 'Q': 2,
    'seasonal_period': 12
...
# === Training & Logging ===
with mlflow.start_run(run_name="SARIMAX-post-challenger") as run:
    try:
        ...
        client.set_registered_model_alias(
            name="sarimax-model",
            alias="post-challenger",
            version=registered_model.version
        )

        print(f"✅ Model registered as version {registered_model.version} with alias 'post-challenger'")
