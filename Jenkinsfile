pipeline {
    ...
    stages {
        ...
        stage('Pre-Production Logic') {
            ...
            steps {
                echo "🔁 Running pre-production retraining and evaluation..."

                // Step 1: Download feature_selection.csv from S3
                sh '''
                    . .venv/bin/activate
                    python "feature selection/download_feature_selection.py"
                '''

                // Step 2: Retrain SARIMAX using pre-challenger config
                sh '''
                    . .venv/bin/activate
                    python "model training/sarimax.py"
                '''

                // Step 3: Evaluate and update alias if needed
                sh '''
                    . .venv/bin/activate
                    python "model training/compare_model.py"
                '''
